...
void f() {
	...
	printMatrix(h_V);
	printMatrixKernel<<<1,1>>>(d_V);
	cudaDeviceSynchronize();
	printf("\n");
	printTranspose(h_Vt);
	printMatrixKernel<<<1,1>>>(d_Vt);
	cudaDeviceSynchronize();
	printf("\n");
	printMatrix(h_U);
	printMatrixKernel<<<1,1>>>(d_U);
	cudaDeviceSynchronize();
	printf("\n");
	printTranspose(h_Ut);
	printMatrixKernel<<<1,1>>>(d_Ut);
	cudaDeviceSynchronize();
	printf("\n");
...
// Prints a matrix from the GPU
__global__ void printMatTKernel(float *V)
{
	if (threadIdx.x == 0)
	{
		for (int j = 0; j < N; j++)
...
__global__ void printMatFlatKernel(float *V)
{
	if (threadIdx.x == 0)
	{
		for (int i = 0; i < N*M; i++)
